...
//                .filter(p -> playerOccupied.stream().noneMatch(p::equals))
                .sorted((a, b) -> {
                    Vec3d playerPos3d = new Vec3d(this.player.getX(), this.player.getY(), this.player.getZ());
                    double aDistance = playerPos3d.squaredDistanceTo(Vec3d.ofCenter(a));
                    double bDistance = playerPos3d.squaredDistanceTo(Vec3d.ofCenter(b));
